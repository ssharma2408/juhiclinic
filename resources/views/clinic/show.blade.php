...
@section('content')
    ...
    <div class="section-title d-flex justify-content-between align-items-center pt-0">
        <h3 class="title">{{ $clinic->name }}</h3>
        <span class="text-descripstion pb-0 mb-0">Package:{{ $clinic->package->package }} </span>
    </div>
    ...
    <div class="card mb-0">
        <div class="card-body">
            <div class="balance-area-bg bg-transaction-details">
                <div class="balance-title text-center section-title">
                    <h3 class="title-lg">{{ $clinic->clinic_admin->name }}</h3>
                </div>
                <div class="ba-balance-inner text-center text-description">
                    <div class="icon">
    ...
    <div class="my-clinic-details">
        <div class="section-title d-flex justify-content-between align-items-center">
            ...
            <a href="{{ route('clinic.closed.edit', ['clinic_admin' => $clinic->clinic_admin->id]) }}"
                class="btn btn-secondary btn-sm">Update</a>
        </div>
        @if (!empty($closed_days))
            @foreach ($closed_days as $day)
                <div
                    class="d-flex justify-content-between align-items-start p-3 mb-2 bg-body-tertiary rounded-3 text-secondary">
