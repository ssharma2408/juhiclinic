                ...
                <div class="card">
                    <div class="card-body">
                        <div class="d-grid gap-2 col-6 mx-auto">
                            <button class="btn btn-primary work_status" id="work_status_{{ $patients['slot_id'] }}"
                                @if ($patients['is_started']) disabled @endif>{{ $patients['is_started'] ? 'Started ...' : 'Start' }}</button>
                            <button class="btn btn-secondary pause" id="pause_{{ $patients['slot_id'] }}">Pause or discard
                                tokens</button>
                        </div>
                        <form class="pause_frm" id="pause_frm_{{ $patients['slot_id'] }}">
                            <div class="pause_action_container text-center py-3"
                                id="pause_action_container_{{ $patients['slot_id'] }}">
                                <input type="radio" class="" value="pause" name="pause_type" required />Pause
                                <input type="radio" class="" value="discard" name="pause_type" required />Diascrd
                                All Tokens
                                <div class="my-3">
                                    <textarea name="message" class="form-control" rows="3" cols="50" placeholder="Pause or discard reason"
                                        required></textarea>
                                </div>
                                <div>
                                    <button type="submit" class="btn btn-primary"
                                        id="pause_action_{{ $patients['slot_id'] }}">Save</button>
                                        ...
                                        <div class="row align-items-center">
                                            ...
                                            <div class="col-md-4 col-12 mt-2">
                                                ...
                                                <select name="status" id="status_{{ $patient->id }}"
                                                    ...
                                                    <option value="0"
                                                        @if ($patient->status == 0) {{ 'selected' }} @endif>Close
                                                    </option>
                                                    <option value="1"
                                                        @if ($patient->status == 1) {{ 'selected' }} @endif>Open
                                                    </option>
                                                    <option value="2"
                                                        @if ($patient->status == 2) {{ 'selected' }} @endif>Hold
                                                    </option>
                                                </select>
                                            </div>
                                            ...
                                            <div class="col-md-4 col-12 mt-2">
                                                ...
                                                <input class="form-control" name="prescription[]" type="file"
                                                    id="prescription_{{ $patient->id }}" multiple="multiple">
                                            </div>
                                            ...
                                            <div class="col-md-6 col-12 mt-2 next_visit">
                                                ...
                                                <input class="form-control" type="date" name="next_visit_date"
                                                    min="<?php echo date('Y-m-d'); ?>" value="{{ old('next_visit_date') }}" />
                                            </div>
                                            <div class="col-md-6 col-12 mt-2 align-self-end">
                                                <button type="submit"
                                                    class="btn_update btn btn-secondary btn-rounded">Update</button>
                                                ...
                                                <div class="patient_msg text-danger small"
                                                    id="msg_{{ $patient->token_id }}"></div>
                                            </div>
                                        </div>
                                    @else
                                        <div class="row align-items-center">
                                            ...
                                            <div class="col-md-6 col-12 mt-2 align-self-end">
                                                ...
                                                <input type="hidden" name="slot_id"
                                                    value="{{ $patient->timing_id }}" />
                                                <input type="hidden" name="is_online"
                                                    value="{{ $patient->is_online }}" />
                                                <div class="patient_msg text-danger small"
                                                    id="msg_{{ $patient->token_id }}"></div>
                                            </div>
                                        </div>
                ...
                success: function(data) {
                    ...
                    if (data.success) {
                        ...
                        if (data.pause_type == "discard") {
                            var html =
                                '<div class="section-title pt-0 mb-2 row d-flex justify-content-between align-items-center"><h1 class="title">Current Appointments</h1>		<div class="text-descripstion text-center mt-4 secondary-text">All tokens are discarded successfully</div></div>';
                            $("#pause_frm_" + frm_id).parent().parent().html(html);
                        } else {
                            setTimeout(
                                function() {
                                    $('#work_status_' + frm_id).text("Start").attr("disabled",
                                        false);
                                    $("#slot_container_" + ele_id).find(".btn_update").attr(
                                        "disabled", true);
                                    $('#pause_' + frm_id).hide();
                        ...
                        $("#slot_msg_" + frm_id).html(
                            "There is a technical error, please try after sometime");
                    }
                },
                cache: false,
                contentType: false,
