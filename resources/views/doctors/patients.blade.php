    ...
    <div class="mb-4">
        <form>
            <div class="input-group search-field">
                ...
                <input type="search" class="form-control" autocomplete="off" autocorrect="off" autocapitalize="off"
                    spellcheck="false" maxlength="512" placeholder="Search patients" aria-label="Search patients"
                    aria-describedby="free-search" value="" id="InputSearch" name="query" />
            </div>
        </form>
    </div>
                ...
                <div class="card-body">
                    ...
                    @foreach ($details['visit_date'] as $date)
                        <div class="d-flex mt-2">
                            ...
                            <button type="button" class="ms-auto btn btn-primary btn-sm show_btn"
                                id="history_{{ $date['history_id'] }}">View</button>
                        </div>
                    @endforeach
                </div>
            </div>
                    ...
                    if (data.success) {
                        ...
                        if (Object.keys(data.patient_arr).length > 0) {
                            ...
                            for (var key in data.patient_arr) {
                                ...
                                patient_html +=
                                    '<div class="card"><div class="card-body"><div class="details fw-bold secondary-text text-uppercase  mb-3">' +
                                    data.patient_arr[key].name + '</div>';
                                for (var k in data.patient_arr[key].visit_date) {

                                    patient_html += '<div class="d-flex mt-2"><div class="visit_date">' + data
                                        .patient_arr[key].visit_date[k].visit_date +
                                        '</div><button type="button" class="ms-auto btn btn-primary btn-sm show_btn" id="history_' +
                                        data.patient_arr[key].visit_date[k].history_id +
                                        '">View</button></div>';
                                }
